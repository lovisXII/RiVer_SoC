...
typedef struct _File_read{
	Elf32_Obj *pObj_struct; // pObj returns by Read_elf
	uint32_t start_adr ;	// start adr of the elf file
	uint32_t kernel_start_adr ; // kernel adr of the elf file
	int size ; //total size of the data/inst in the elf file
...
/*
	This function allow you to get the data contain at a specify adress
*/
{
	Seg *ps = TabSeg;
...
typedef struct _section_intel{
	int start_adr ;
	int size ;
}SECTION_INTEL;

void get_section_intel(char *filename, Elf32_Obj *pObj, char *name, SECTION_INTEL* intels)
/*
This function take as an argument the namme of a section
It returns the containt of the section (adress+data) in the file name filename
*/
{
	char *filename_complet = calloc(512*sizeof(char),512*sizeof(char));
	strcat(filename_complet, filename);
	strcat(filename_complet, "_");
	strcat(filename_complet, name);
	strcat(filename_complet, ".txt");
	FILE *output  = fopen(filename_complet, "w");

	intels->start_adr = 0;
	intels->size = 0;
	for (int i=0; i< pObj->Head.e_shnum; i++)
	{
		if(strcmp(pObj->Section_name[i],name) == 0)
		{
			intels->start_adr = pObj->Section_Hdr[i]->sh_addr;
			intels->size = (int) pObj->Section_Hdr[i]->sh_size;
		}
	}	
	if(intels->start_adr == 0 && intels->size == 0)
		fprintf(stderr,"Section given doesnt exist\n");
	if (output == NULL ) 
	{   
		...
		for(int i = 0; i < intels ->  size; i+=4){
				fprintf(output,"%x, %x\n",intels->start_adr+i, mem_lw(intels->start_adr+i));
		}
	}
...
/*
Allow to pars any kind of elf file
Return a structure containing usefull intels
*/
