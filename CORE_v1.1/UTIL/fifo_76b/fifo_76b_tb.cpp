...
{
    //testbench signals :
    sc_signal< sc_bv<76> > din ;
    sc_clock clk("clk",1,SC_NS) ;
    sc_signal<bool> push, pop ;
    sc_signal<bool> reset_n ;

    sc_signal <bool> full, empty ;
    sc_signal< sc_bv<76> > dout ;

    //entity declaration :
    fifo_76b fifo_76b0("fifo_76b0") ;
    ...
    fifo_76b0.PUSH(push) ;
    fifo_76b0.POP(pop) ;
    fifo_76b0.RESET_N(reset_n) ;
    fifo_76b0.FULL(full) ;
    fifo_76b0.EMPTY(empty) ;
    fifo_76b0.DOUT(dout) ;
    ...
    for(int i =0 ; i < 15 ; i++)
    {
        ...
        cerr   << "fifo_v : "<< fifo_76b0.fifo_v.read() << endl ;
