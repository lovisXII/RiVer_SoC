...
{
    //testbench signals :
    sc_signal< sc_bv<78> > din ;
    sc_clock clk("clk",1,SC_NS) ;
    sc_signal<bool> push, pop ;
    sc_signal<bool> reset_n ;

    sc_signal <bool> full, empty ;
    sc_signal< sc_bv<78> > dout ;

    //entity declaration :
    fifo_78b fifo_78b0("fifo_78b0") ;
    ...
    fifo_78b0.PUSH(push) ;
    fifo_78b0.POP(pop) ;
    fifo_78b0.RESET_N(reset_n) ;
    fifo_78b0.FULL(full) ;
    fifo_78b0.EMPTY(empty) ;
    fifo_78b0.DOUT(dout) ;
    ...
    for(int i =0 ; i < 15 ; i++)
    {
        ...
        cerr   << "fifo_v : "<< fifo_78b0.fifo_v.read() << endl ;
