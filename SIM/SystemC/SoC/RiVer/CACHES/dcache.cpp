...
void dcache::adresse_parcer()
{
  ...
  sc_uint<32> DT_A_BUS = ADR_SW.read();
...
void dcache::miss_detection()
{
  ...
  if(address_tag == w0_TAG[address_index.read()])
  {
    ...
    {
      DATA_SDC.write(w0_word[address_index.read()][address_offset.read()/4]);
    }
  }
  else
    way0_hit = false;
  ...
  if(address_tag == w1_TAG[address_index.read()])
  {
    ...
    {
      DATA_SDC.write(w1_word[address_index.read()][address_offset.read()/4]);
    }
  }
  else
    way1_hit = false;
  ...
  STALL_SDC.write((miss && LOAD_SM) || (STORE_SM && full));
...
void dcache::mae_output()
{
  ...
  switch(current_state.read())
  {
    ...
    case DC_WAIT_MEM:
      ...
      {
        DTA_VALID_SDC = false;
        sc_uint<32> DT_A_MP = A_SDC.read();
        if(LRU_bit_check[DT_A_MP.range(10,4)])
        {
          w0_word[DT_A_MP.range(10,4)][burst_cpt++] = DT_SW.read();
          w0_TAG[DT_A_MP.range(10,4)] = DT_A_MP.range(31,11);
          w0_LINE_VALIDATE[DT_A_MP.range(10,4)] = true;
        }
        else
        {
          w1_word[DT_A_MP.range(10,4)][burst_cpt++] = DT_SW.read();
...
void dcache::trace(sc_trace_file* tf)
{
  ...
  sc_trace(tf, DATA_SM, GET_NAME(DATA_SM));
  sc_trace(tf, STALL_SDC, GET_NAME(STALL_SDC));

  // interfaz bus
  sc_trace(tf, READ_SDC, GET_NAME(READ_SDC)); 
  sc_trace(tf, WRITE_SDC, GET_NAME(WRITE_SDC));
  sc_trace(tf, DT_SDC, GET_NAME(DT_SDC));
  sc_trace(tf, A_SDC, GET_NAME(A_SDC));
  ...
  sc_trace(tf, SIZE_SDC, GET_NAME(SIZE_SDC));

  sc_trace(tf, DTA_VALID_SDC, GET_NAME(DTA_VALID_SDC));
