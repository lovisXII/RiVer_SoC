...
SC_MODULE(exec) {
    ...
    sc_in<sc_uint<32>>
                       OP1_RD;  // can contains CSR if CSR_type_operation_RD == 1
    ...
    sc_in<bool>        NEG_OP2_RD, WB_RD, MEM_SIGN_EXTEND_RD,
    ...
    SC_CTOR(exec)
        : alu_inst("alu"), shifter_inst("shifter"), fifo_inst("exe2mem") {
        // ALU port map :
        ...
        sensitive << bp_mem_data_sd << DEST_RD << MEM_SIZE_RD << MEM_LOAD_RD
                  << MEM_SIGN_EXTEND_RD << MEM_STORE_RD << WB_RD << exe_res_se;
        SC_METHOD(fifo_unconcat);
        sensitive << exe2mem_dout_se;
        SC_METHOD(manage_fifo);
        sensitive << exe2mem_full_se << DEC2EXE_EMPTY_SD << OP1_VALID_RD
                  << OP2_VALID_RD << blocked;
        SC_METHOD(bypasses);
        sensitive << OP1_VALID_RD << OP2_VALID_RD << MEM_DEST_RM << MEM_RES_RM
                  << DEST_RE << EXE_RES_RE << RADR1_RD << RADR2_RD << OP1_RD
                  << OP2_RD << MEM_DATA_RD;
