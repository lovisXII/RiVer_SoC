...
int sc_main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        if (registers[radr1_] != (int)radr1_data.read()) {
            cerr << "Error : register mismatch in register " << radr1_ << "from port 1, expected " << registers[radr1_]
                 << " got " << (int)radr1_data.read()

                 << endl;
            exit(1);
        }
        if (registers[radr2_] != (int)radr2_data.read()) {
            cerr << "Error : register mismatch in register " << radr2_ << " from port 2, expected " << registers[radr2_]
                 << " got " << (int)radr2_data.read()

                 << endl;
            exit(1);
        }
        if (registers[32] != (int)read_pc.read()) {
            cerr << "Error : register mismatch in register pc, expected " << registers[32] << " got "
                 << (int)read_pc.read() << endl;
            exit(1);
        }
        if (valid[radr1_] != (int)radr1_valid.read()) {
            cerr << "Error : register validity mismatch in register " << radr1_ << " from port 1, expected "
                 << valid[radr1_] << " got " << (int)radr1_valid.read() << endl;
            exit(1);
        }
        if (valid[radr2_] != (int)radr2_valid.read()) {
            cerr << "Error : register validity mismatch in register " << radr2_ << " from port 2, expected "
                 << valid[radr2_] << " got " << (int)radr2_valid.read() << endl;
            exit(1);
        }
        // cout << "--------------------------------"
        << endl;
        ...
        << inc_pc_valid_ << endl;
        // cout << "Data read 1 :"
        << radr1_data.read() << endl;
        // cout << "Data read 2 : "
        << radr2_data.read() << endl;
        // cout << "ADR_DEST :"
        << adr_dest_ << endl;
        // cout << "inval_dest : "
        << inval_dest_ << endl;
        ...
        //     s = "r"+ to_string(i) ;
        //     cout
        << s << " ,value : " << reg_inst.REG[i].read() << endl;
        // }
    }
