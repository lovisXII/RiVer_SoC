...
{
    RDATA1_SR.write(REG_RR[RADR1_SD.read()]) ; //on charge la donnée dans REG[index] dans le port de lecture 1
    RDATA2_SR.write(REG_RR[RADR2_SD.read()]) ; //on charge la donnée dans REG[index] dans le port de lecture 2
    if (RESET_N.read()) {
...
void reg::writing_adresse()
{
    ...
    while(1)
    {
        ...
        if (WRITE_PC_ENABLE_SD.read()) {
            REG_RR[32].write(WRITE_PC_SD) ; // we write the data into the register from the written adress
        }
        if (not(WADR_SW.read() == 32 && WRITE_PC_ENABLE_SD.read())) {
            if(WADR_SW.read() != 0) // si on cherche à écrire dans le registre 0 on ne fait rien
            {
                ...
                {
                REG_RR[WADR_SW.read()].write(WDATA_SW) ; // we write the data into the register from the written adress
                } 
            }
        }
