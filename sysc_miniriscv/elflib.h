...
// a source buffer
{
    ...
    if (ps == NULL) {
        if ((TabSeg = (Seg*)malloc(sizeof(Seg))) == NULL)
            return 0;  // malloc return NULL if allocation isn't possible
        if ((TabSeg->ra = malloc(size)) == NULL) return 0;  // same
        if (src)
            memcpy(TabSeg->ra,
                   src,
                   size);  // memcpy allow to copy a memory bloc from a source to a destination
        TabSeg->va   = adr;
        TabSeg->size = size;
        TabSeg->next = NULL;
        return 1;
    ...
    if (adr < ps->va) {
        ...
        if ((TabSeg = (Seg*)malloc(sizeof(Seg))) == NULL) return 0;
    ...
    if ((new_seg = (Seg*)malloc(sizeof(Seg))) == NULL) return 0;
...
FILE_READ* Read_Elf32(const char* file_name) {
    FILE*      felf;
    Elf32_Obj* pObj;
    ...
    pObj            = (Elf32_Obj*)malloc(sizeof(Elf32_Obj));
    pObj->Shead_tab = NULL;
    ...
    if (pObj->Head.e_shnum != 0) {
        ...
        pObj->Shead_tab = (Elf32_Shdr*)malloc(pObj->Head.e_shnum * sizeof(Elf32_Shdr));
        ...
        pObj->SecName    = (char*)malloc(pObj->SecNameHdr->sh_size);
        ...
        pObj->Section_name = (char**)calloc((pObj->Head.e_shnum) * sizeof(char),
                                            (pObj->Head.e_shnum) * sizeof(char));
        pObj->Section_Hdr  = (Elf32_Shdr**)calloc((pObj->Head.e_shnum) * sizeof(Elf32_Shdr),
                                                 (pObj->Head.e_shnum) * sizeof(Elf32_Shdr));
        pObj->Sec          = (uint32_t**)calloc((pObj->Head.e_shnum) * sizeof(uint32_t),
                                       (pObj->Head.e_shnum) * sizeof(uint32_t));
        pObj->size         = (size_t*)calloc((pObj->Head.e_shnum) * sizeof(size_t),
                                     (pObj->Head.e_shnum) * sizeof(size_t));
        ...
        pObj->SymName = (char*)malloc(pObj->SecSymNameHdr->sh_size * sizeof(char));
        fseek(felf, pObj->SecSymNameHdr->sh_offset, SEEK_SET);
        fread(pObj->SymName, sizeof(char), pObj->SecSymNameHdr->sh_size, felf);

        pObj->SecSym = (Elf32_Sym*)malloc(pObj->SecSymHdr->sh_size);
        ...
        for (int i = 0; i < pObj->Head.e_shnum; i++) {
            if (pObj->Section_Hdr[i]->sh_addr != 0) {
                if (mem_add_seg(pObj->Section_Hdr[i]->sh_addr,
                                pObj->Section_Hdr[i]->sh_size,
                                pObj->Sec[i]) == 1) {
                    fprintf(stdout,
                            "Chargement du segment %s adr = 0x%8x, total size : 0x%x\n",
                            pObj->Section_name[i],
                            pObj->Section_Hdr[i]->sh_addr,
                            pObj->Section_Hdr[i]->sh_size);
        ...
        if (return_structure == NULL) {  // printf("Structure return is null\n");
        }
